...
timeStepsBeforeReMesh = [2 5 10 20 50];
numSimulations = length(timeStepsBeforeReMesh);
numSteps = 5000;
...
title('RMSE Over Time for Various Remesh Intervals (and Initial Conditions)');
xlabel('Time Step');
ylabel('Root Mean Square Error');
legendLabels = arrayfun(@(x) sprintf('Remesh every %d (Case %d)', timeStepsBeforeReMesh(x), x), 1:numSimulations, 'UniformOutput', false);
