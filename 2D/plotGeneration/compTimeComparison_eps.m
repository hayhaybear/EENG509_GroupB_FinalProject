...
for i = 1:numSimulations
    ...
    timeOptimized = tic; 
    for t = 1:numSteps
        if mod(t, timeStepsBeforeReMesh(2)) == 1 || t == 1
            [~,~,grid_mask] = newGrid2d_v2(0, h_optimized, epsilon(i), 4, 0); 
...
% Plot 1: RMSE
subplot(2,1,1);
plot(epsilon, bigRMSE, '-o', 'LineWidth', 2);
title('RMSE vs. Threshold');
ylabel('RMSE');
grid on;

% Plot 2: Time Comparison
subplot(2,1,2);
plot(epsilon, bigTimeElapsed, '-s', 'DisplayName', 'Standard Time');
hold on;
plot(epsilon, bigTimeElapsed_optimized, '--^', 'DisplayName', 'Optimized Time');
title('Computation Time vs. Threshold');
xlabel('Epsilon');
ylabel('Time (s)');
