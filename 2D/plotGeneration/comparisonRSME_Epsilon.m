% This code is a combination of other code to plot
...
for i = 1:numSimulations
    ...
    for t = 1:numSteps
        if mod(t, timeStepsBeforeReMesh) == 1 || t == 1
            [~,~,grid_mask] = newGrid2d_v2(0, h, epsilon(i), 4, 0); 
        ...
        if hit_top || hit_bottom || hit_left || hit_right
            fprintf('Boundary hit detected at timestep %d (Epsilon: %.2f)\n', t, epsilon(i));
            break; 
        end
...
% Google Gemini refined this plotting logic
figure;
plot(1:numSteps, bigRMSE', 'LineWidth', 1.5); 
title(sprintf('RMSE Over Time for Various Epsilon Values (Remesh Interval: %d)', timeStepsBeforeReMesh));
xlabel('Time Step');
ylabel('Root Mean Square Error');

% Generate dynamic legend labels based on the epsilon array
legendLabels = arrayfun(@(x) sprintf('Epsilon = %.2f', epsilon(x)), 1:numSimulations, 'UniformOutput', false);
...
function [h, hu, hv] = rk4_step_optimized(h, hu, hv, dt, dx, dy, g_prime, f, lambda, active_indexes)
    ...
    [h2, hu2, hv2] = apply_boundaries_optimized(h2, hu2, hv2); 
    ...
    [h3, hu3, hv3] = apply_boundaries_optimized(h3, hu3, hv3); 
    ...
    [h4, hu4, hv4] = apply_boundaries_optimized(h4, hu4, hv4); 
